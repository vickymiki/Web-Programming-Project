...
async function validateParameters(
  ...
  if (await checkIfAddressExists(address, city, state, zip))
    throw "A restaurant with that address already exists";
...
function validateObjectId(id) {
  if (typeof id !== "string" && !ObjectId.isValid(id))
    throw `id must be of type string or ObjectId: ${id}`;
  if (typeof id === "string" && id.length === id.split(" ").length - 1)
    throw `id as string must not be empty: ${id}`;
  if (typeof id === "string") {
    try {
      id = ObjectId(id);
    } catch (e) {
...
async function addRestaurant(
  ...
  let restaurantObj = await validateParameters(
    name,
    address,
    city,
    state,
...
async function checkIfAddressExists(address, city, state, zip) {
  const restaurantCollection = await restaurants();
  const restQuery = await restaurantCollection.findOne({
    streetAddress: address,
    city,
    state,
    zip,
...
async function getRestaurantsManagedByUser(username) {
  ...
  const restaurantCollection = await restaurants();
  let restQuery = await restaurantCollection
    .find({ managerUsername: username })
    .toArray();
  restQuery.forEach((x) => {
...
async function removeRestaurant(restaurant_id) {
  ...
  if (deleteRest.deletedCount === 0)
    throw `Failed to delete restaurant with id: ${id}`;
...
async function addFood_Item(restaurant_id, foodItem) {
  ...
  let update = await restaurantCollection.updateOne(
    { _id: restaurant_id },
    { $push: { menuItems: foodItem } }
  );
...
async function removeFood_Item(restaurant_id, foodItem_id) {
  ...
  let update = await restaurantCollection.updateOne(
    { _id: restaurant_id },
    { $pull: { menuItems: { _id: foodItem_id } } }
  );
  if (update.matchedCount === 0)
    throw `Failed to remove food item: ${foodItem_id}`;
...
module.exports = {
  addRestaurant,
  getRestaurantIdFromName,
  addFood_Item,
  removeFood_Item,
  getAllResaurants,
  getRestaurantFromId,
  getRestaurantsManagedByUser,
};
